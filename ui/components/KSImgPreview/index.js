...
export default class KSImgPreview extends React.Component {
     ...
     * 压缩图片
     */
    handleImgCompress(originImgUrl, width) {
    ...
    getContainerNode() {
        ...
        if (children) {
            ...
            let thumbnailStyle = typeof thumbnail === 'boolean' ? { width: 30, height: 30 } : thumbnail,
                imgSrc = imgPreviewUrl || src;
            const { width = 30 } = thumbnailStyle;
            return (
                <div onClick={this.changeVisible}>
                    <img style={{...thumbnailStyle, cursor: 'pointer'}} src={this.handleImgCompress(imgSrc, width)} />
                </div>
            );
        } else {
